...
function NewsletterForm() {
  ...
  return (
    <div className="flex flex-col space-y-8 md:w-[400px]">
      <form onSubmit={handleSubmit} className="newsletter-form mt-10 animate-fade-in-3">
        <div className="group flex items-center gap-x-4 py-1 pl-4 pr-1 rounded-[9px] bg-[#c4cbd1] hover:bg-[#8178b8] shadow-outline-gray hover:shadow-transparent focus-within:bg-[#a19ead] focus-within:!shadow-outline-gray-focus transition-all duration-300">
          ...
          <input
            ...
            required
            type="email"
            className="flex-1 text-white text-sm sm:text-base outline-none group-focus-within:placeholder-white bg-transparent placeholder:transition-colors placeholder:duration-300"
          />
          <button
            ref={buttonRef}
            className={`${
        ...
        {(successMessage || errorMessage) && (
          <div className="flex items-start space-x-2 bg-[#0A0E12] shadow-outline-gray text-white rounded-[9px] py-4 px-6 animate-fade-bottom absolute">
            ...
            <div className="text-xs sm:text-sm text-[#4B4C52]">
              {successMessage ? (
                <p>
                  We&apos;ve added <span className="text-[#ADB0B1]">{successMessage.email_address}</span> to
                  our waitlist. We&apos;ll let you know when we launch!
                </p>
              ) : (
                <p>You are already added to our waitlist. We&apos;ll let you know when we launch!</p>
              )}
            </div>
            <XMarkIcon
